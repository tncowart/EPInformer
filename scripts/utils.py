...
def prepare_input(gene_enhancer_table, gene_list, cell, num_features=3):
    ...
    mRNA_feauture = pd.read_csv(
        "./data/mRNA_halflife_features.csv", index_col="gene_id"
    )
    if cell == "K562":
        promoter_signals = pd.read_csv(
            "./data/K562_DNase_ENCFF257HEE_hic_4DNFITUOMFUQ_1MB_ABC_nominated/DNase_ENCFF257HEE_Neighborhoods/GeneList.txt",
            sep="\t",
            index_col="symbol",
        )
        promoter_signals["PromoterActivity"] = np.sqrt(
            promoter_signals["H3K27ac.RPM.TSS1Kb"] * promoter_signals["DHS.RPM.TSS1Kb"]
        )
    elif cell == "GM12878":
        promoter_signals = pd.read_csv(
            "./data/GM12878_DNase_ENCFF020WZB_hic_4DNFI1UEG1HD_1MB_ABC_nominated/DNase_ENCFF020WZB_Neighborhoods/GeneList.txt",
            sep="\t",
            index_col="symbol",
        )
        promoter_signals["PromoterActivity"] = np.sqrt(
    ...
    for gene in tqdm(gene_list):
        ...
        PE_code, activity_list, distance_list, contact_list, gene_name, PE_links = (
        ...
        mRNA_promoter_feat = np.array(
            list(mRNA_feauture.loc[gene, mRNA_feats].values)
            + [promoter_signals.loc[gene, "PromoterActivity"]]
        )
        if num_features == 1:
            PE_feat = distance_list[:, np.newaxis]
            mRNA_promoter_feat = np.array(
                list(mRNA_feauture.loc[gene, mRNA_feats].values) + [0]
            )
        elif num_features == 2:
            PE_feat = np.concatenate(
                [
                    distance_list[:, np.newaxis],
                    activity_list[:, np.newaxis],
                ],
                axis=-1,
            )
        else:
...
def encoder_promoter_enhancer_CRISPRi(
    pe_df, hg19_fasta_path="./data/hg19.fa", verbose=True, HiC_norm=False
):
    ...
    RNA_feats = pd.read_csv(
        "./data/GM12878_K562_18377_gene_expr_fromXpresso.csv",
    ...
    promoter_df = pd.read_csv(
        "./data/CRISPRi-FlowFISH_Fulco2019/DNase_ENCFF257HEE_Neighborhoods/GeneList.txt",
        sep="\t",
        index_col="symbol",
    )
    promoter_df["PromoterActivity"] = np.sqrt(
        promoter_df["H3K27ac.RPM.TSS1Kb"] * promoter_df["DHS.RPM.TSS1Kb"]
    )
    ...
    for idx, row in pe_df.iterrows():
        ...
        enhancers_code[e_i][code_start : code_start + enhancer_len] = one_hot_encode(
            hg19_fasta_extractor.extract(enhancer_target_interval)
        )
    ...
    rna_ts = np.array(list(RNA_feats.loc[eid].values) + [singleGene_promoter_activity])[
        np.newaxis, :
    ]
    pe_feat = np.concatenate(
        [pe_distance[:, np.newaxis], pe_activity[:, np.newaxis], pe_hic[:, np.newaxis]],
        axis=-1,
    )
...
def compute_enhancer_gene_attention(model, pe_df, use_hic=False, device="cpu"):
    ...
    model.eval()
    with torch.no_grad():
        seq_input, feat_input, rna_input = encoder_promoter_enhancer_CRISPRi(
            pe_df, hg19_fasta_path="../hg19.fa", verbose=False, HiC_norm=True
        )
        ...
        if not use_hic:
            ...
            feat_input = torch.cat(
                [feat_input[:, :, :1], feat_input[:, :, 2:], feat_input[:, :, 1:2]],
                dim=-1,
            )
...
def predict_enhancer_activity(
    enhancer_model, chrom, position, window_size=1024, stride=128, device="cuda"
):
    ...
    center = position
    print(
        "The extened enhancer region: {}:{}-{}".format(
            chrom, center - window_size, center + window_size
        )
    )
    pred_list = []
    x_list = []
    ...
    info_df = pd.DataFrame(
        info_list, columns=["chrom", "start", "end", "seq", "enhancer_mid", "pred"]
    )
    return info_df
...
def perturb_enhancer(model, pe_df, use_hic=False, device="cpu"):
    ...
    pe_df["change of predicted expression"] = (
        (perturb_pred_list - all_expr)
    ) / all_expr
