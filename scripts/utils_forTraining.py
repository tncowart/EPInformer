...
def train(
    ...
    trainloader = data_utils.DataLoader(
        train_ds, batch_size=batch_size, shuffle=True, num_workers=5, pin_memory=True
    )
    ...
    for epoch in range(EPOCHS):
        ...
        val_mse_all, val_r2_all, val_pr_all = validate(
            net, valid_ds, n_enhancers=n_enhancers, device=device
        )
        val_r2 = val_r2_all
...
def validate(
    net,
    valid_ds,
    net_type="seq_feat_dist",
    n_enhancers=50,
    batch_size=16,
    device="cuda",
):
    validloader = data_utils.DataLoader(
        valid_ds, batch_size=batch_size, pin_memory=True, num_workers=0
    )
    net.eval()
    L_expr = nn.MSELoss()
...
def test(
    ...
    testloader = data_utils.DataLoader(
        test_ds, batch_size=batch_size, pin_memory=True, num_workers=0
    )
...
class promoter_enhancer_dataset(Dataset):
    def __init__(
        self,
        data_folder="/content/drive/MyDrive/EPInformer/github/EPInformer/data/",
        expr_type="CAGE",
        usePromoterSignal=True,
        first_signal="distance",
        signal_type="H3K27ac",
        cell_type="K562",
        ...
        self.expr_df = pd.read_csv(
            self.data_folder + "/GM12878_K562_18377_gene_expr_fromXpresso.csv",
            index_col="ENSID",
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.distance_threshold is not None:
            ...
            enhancers_zero[abs(enhancer_distance) < self.distance_threshold] = (
                enhancers_code[abs(enhancer_distance) < self.distance_threshold]
            )
            enhancers_code = enhancers_zero

            enhancer_distance_zero = np.zeros_like(enhancer_distance)
            enhancer_distance_zero[abs(enhancer_distance) < self.distance_threshold] = (
                enhancer_distance[abs(enhancer_distance) < self.distance_threshold]
            )
        ...
        if self.hic_threshold is not None:
            ...
            enhancers_zero = np.zeros_like(enhancers_code)
            enhancers_zero[enhancer_contact > self.hic_threshold] = enhancers_code[
                enhancer_contact > self.hic_threshold
            ]
            enhancers_code = enhancers_zero

            enhancer_contact_zero = np.zeros_like(enhancer_contact)
            enhancer_contact_zero[
                enhancers_code[enhancer_contact > self.hic_threshold]
            ] = enhancer_contact[enhancers_code[enhancer_contact > self.hic_threshold]]
        ...
        if self.n_extraFeat == 1:
            ...
            pe_feat = np.concatenate(
                [pe_distance[:, np.newaxis], pe_activity[:, np.newaxis]], axis=-1
            )
        elif self.n_extraFeat == 3:
            pe_feat = np.concatenate(
                [
                    pe_distance[:, np.newaxis],
                    pe_hic[:, np.newaxis],
                    pe_activity[:, np.newaxis],
                ],
                axis=-1,
            )
        else:
        ...
        enhancers_code_tensor = torch.from_numpy(
            enhancers_code[: self.n_enhancers, :]
        ).float()
        ...
        if self.expr_type == "CAGE":
            cage_expr = np.log10(
                self.expr_df.loc[sample_ensid][self.cell_type + "_CAGE_128*3_sum"] + 1
            )
